...
module Trino::Client
  ...
  # to explicitly reset the memoized faraday clients
  def self.reset!
    @faraday_clients = {}
  end

  def self.build_faraday_client(options)
  ...
  private_class_method :faraday_ssl_options, :build_faraday_client, :encode_properties, :encode_client_info, :encode_client_tags
